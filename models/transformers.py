...
if __name__ =='__main__':
    ...
    src_seq_len = 50        # Maximum length of source sequences
    tgt_seq_len = 50        # Maximum length of target sequences
    ...
    # Create dummy masks (assuming no padding for simplicity)
    src_mask = torch.ones(batch_size, 1, src_seq_len)  
    tgt_mask = torch.ones(batch_size, 1, tgt_seq_len)

    # Forward pass through the model
    encoder_output = transformer.encode(src_seq, src_mask)
    decoder_output = transformer.decode(encoder_output, src_mask, tgt_seq, tgt_mask)
